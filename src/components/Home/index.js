import {useContext} from 'react'
import MoodTrackerContext from '../../context/MoodTrackerContext'
import Header from '../Header'
import Calender from '../Calender'
import './index.css'

const Home = () => {
  const {
    emojisListNew,
    nameDayCount,
    activeEmoji,
    calenderList,
    daysList,
    activeEmojiName,
    activeDay,
    onEmojiClick,
    onEmojiNameChange,
    onDayChange,
  } = useContext(MoodTrackerContext)

  return (
    <>
      <Header />

      <div data-testid="homeBodyContainer" className="home-body-container">
        <h1 data-testid="homeMainHeading" className="home-main-heading">
          Moods in a Month
        </h1>

        <div data-testid="homeAllContainer" className="home-all-container">
          <div data-testid="calenderContainer1" className="calender-container1">
            <Calender calenderList={calenderList} />
          </div>
          <div data-testid="helloMoto" className="helloMoto">
            <div
              data-testid="emojisListContainer"
              className="emojis-list-container"
            >
              <ul data-testid="emojiUl" className="emoji-ul">
                {emojisListNew.map(({id, emojiName, emojiUrl}) => {
                  const isActive = activeEmoji === id

                  return (
                    <li data-testid="emojiLi" key={id} className="emoji-li">
                      <button
                        data-testid="emojiButton"
                        type="button"
                        onClick={() => onEmojiClick(id)}
                        className={`emoji-button ${
                          isActive ? 'activeEmojiButton' : ''
                        }`}
                      >
                        <p
                          data-testid="emojiPara"
                          className={`emoji-para ${
                            isActive ? 'activeEmojiPara' : ''
                          }`}
                        >
                          {emojiName}
                        </p>
                        <img
                          data-testid="emojiImg"
                          className={`emoji-img ${
                            isActive ? 'activeEmoji' : ''
                          }`}
                          src={emojiUrl}
                          alt={emojiName}
                        />
                      </button>
                    </li>
                  )
                })}
              </ul>
            </div>
            <div data-testid="selectContainer" className="select-container">
              <select
                data-testid="emojiSelect"
                className="emoji-select"
                value={activeEmojiName}
                onChange={onEmojiNameChange}
              >
                {emojisListNew.map(item => (
                  <option key={item.id} value={item.emojiName}>
                    {item.emojiName}
                  </option>
                ))}
              </select>

              <select
                data-testid="daySelect"
                className="day-select"
                value={activeDay}
                onChange={onDayChange}
              >
                {daysList.map(item => (
                  <option key={item.id} value={item.dayNumber}>
                    {item.day}
                  </option>
                ))}
              </select>

              <h1 data-testid="nameDayCount" className="name-day-count">
                {nameDayCount}
              </h1>
            </div>
          </div>
        </div>
      </div>
    </>
  )
}

export default Home
